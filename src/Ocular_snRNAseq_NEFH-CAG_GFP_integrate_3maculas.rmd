---
title: "Ocular_snRNAseq_NEFH-CAG_GFP_integrate_NEFHs"
output:
  html_document:
    toc: true
    toc_float: true
...
# Abstract
Integrated data from the two new samples (GTSAMPLE1702 and GTSAMPLE1703) with promoter identity in meta data set to NEFH and CAG for comparisons.
...
setwd('~/Ocular_snRNAseq_NEFH-CAG_GFP_Newmacula/NEFH-CAG_GFP_3maculas/')
set.seed(123)
```
...
# previous macula using NEFH
rds_path2 <- list.files('../../Ocular_snRNAseq_RGC_NEFH-GFP_CAG/nf-core/output/cellranger/cellranger/mtx_conversions/',
...
# combine samples
seuratObj <- merge(rds_list[[1]], y = rds_list[2:23], add.cell.ids = names(rds_list), project = "Ocular_NEFH_vs_CAG")
...
# add groups
group <- ifelse(grepl('GTSAMPLE1703', seuratObj$sample), "CAG", "NEFH")
...
gfp_expression <- FetchData(seuratObj.integrated, vars = "GFP")
...
# add groups
group <- ifelse(grepl("GTSAMPLE1703", RGC.integrated$sample), "CAG", "NEFH")
...
# try 0.3-0.5, give 26, 28, 30 clusters
RGC.integrated <- FindClusters(RGC.integrated, resolution = 0.02)
table(RGC.integrated$seurat_clusters)
```
...
cluster_to_celltype <- c('0' = 'Midget OFF',  '1' = 'Midget ON', '2' = 'Other', 
    '3' = 'Other', '4' = 'Parasol OFF', '5' = "Parasol ON")
...
gfp_pos_GRC <- subset(RGC.integrated, subset = GFP > 0)
gfp_neg_GRC <- subset(RGC.integrated, subset = GFP == 0)
