---
title: "Ocular_snRNAseq_macula_comp_dissections"
output:
  html_document:
    toc: true
    toc_float: true
...
# Abstract
The purpose of this project was to compare any differences in cellular composition between two dissection strategies on macula samples.  

The sample IDs are as follows. We processed four lanes for each sample.  
GTSAMPLE1051	MB2366 Macular Retina Punch Left (OS)  
GTSAMPLE1052	MB2366 Macular Retina Punch Right (OD)  
...
setwd('~/Ocular_snRNAseq_macula_comp_dissections')
set.seed(123)
```
...
# combine samples
seuratObj <- merge(rds_list[[1]], y = rds_list[2:8], add.cell.ids = names(rds_list), project = "Ocular_macula_comp_dissections")
# add sample names
cells <- rownames(seuratObj@meta.data)

seuratObj$sample <- sub("(GTSAMPLE105[12]-[1-4]).*", "\\1", cells)

# add groups
group <- ifelse(grepl("GTSAMPLE1051", seuratObj$sample), "GTSAMPLE1051", "GTSAMPLE1052")
