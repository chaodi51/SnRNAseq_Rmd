---
title: "Ocular_snRNAseq_RGC_NEFH-GFP_integrate_periphery_Newmacula_reps"
output:
  html_document:
    toc: true
    toc_float: true
...
# Abstract
Integrate RGC subset from the NEFH lanes with our previous data using this promoter (two eyes periphery and macula).  
RGC subset from GTSAMPLE1702 together with previous RGC data from GTSAMPLE1488 and GTSAMPLE1539
...
setwd('~/Ocular_snRNAseq_NEFH-CAG_GFP_Newmacula')
set.seed(123)
```
...
```{r, eval = FALSE}
# new macula rep (novaseq run)
rds_path1 <- list.files('./nf-core/output/cellranger/cellranger/mtx_conversions/',
    pattern = "GTSAMPLE1702-[1-8]_matrix\\.rds", full.names = TRUE)
rds_names1 <-  gsub("_matrix.rds$", "", basename(rds_path1))
rds_list1 <- lapply(rds_path1, readRDS)
names(rds_list1) <- rds_names1

# periphery
rds_path2 <- list.files('../Ocular_snRNAseq_RGC_NEFH-GFP_periphery/nf-core/output/cellranger/cellranger/mtx_conversions/',
...
# new macula
rds_path3 <- list.files('../Ocular_snRNAseq_RGC_NEFH-GFP_Newmacula/nf-core/output/cellranger/cellranger/mtx_conversions/',
    pattern = "GTSAMPLE1539-[1-8]_matrix\\.rds", full.names = TRUE)
...
# combine samples
seuratObj <- merge(rds_list[[1]], y = rds_list[2:23], add.cell.ids = names(rds_list), project = "ocular_NEFH-GFP_integration")
# add sample names
...
# add groups
group <- ifelse(grepl("GTSAMPLE1702", seuratObj$sample), "Newmacula_rep",
            ifelse(grepl('GTSAMPLE1488', seuratObj$sample), "periphery", "Newmacula"))
...
VlnPlot(seuratObj.integrated, features = c("ENSMFAG00000036911", "SLC17A6", "SNCG","SYT2","GFP"), pt.size = 0.01, ncol=3, combine = TRUE)
```
...
Newmacula_rep <- subset(seuratObj, idents = 'Newmacula_rep')
Newmacula <- subset(seuratObj, idents = 'Newmacula')
periphery <- subset(seuratObj, idents = 'periphery')

```

### redo normalization and annotation for the datasets seperately
#### New macula replicate

```{r}
Newmacula_rep <- qread("Newmacula_rep.qs")
```

```{r, eval = FALSE}
Newmacula_rep <- SCTransform(Newmacula_rep, method = "glmGamPoi", vst.flavor = "v2",
        vars.to.regress = c("percent.mt", "S.Score", "G2M.Score"), verbose = TRUE)

Newmacula_rep <- RunPCA(Newmacula_rep, npcs = 40)
# ElbowPlot(seuratObj, ndims = 40)
Newmacula_rep <- RunUMAP(Newmacula_rep, reduction = "pca", dims = 1:40)
Newmacula_rep <- FindNeighbors(Newmacula_rep, reduction = "pca", dims = 1:40)
Newmacula_rep <- FindClusters(Newmacula_rep, resolution = 0.1)
table(Newmacula_rep$seurat_clusters)

Newmacula_rep <- sctype(Newmacula_rep)
```

```{r, fig.height=10, fig.width=12}
DimPlot(Newmacula_rep, reduction = "umap", label = TRUE, repel = TRUE, group.by = 'celltypes') + theme(legend.position = "top")
DimPlot(Newmacula_rep, reduction = "umap", group.by = "sample") + theme(legend.position = "top")
DimPlot(Newmacula_rep, reduction = "umap", label = TRUE, split.by = "sample", group.by = 'celltypes', ncol = 4) + theme(legend.position = "top")
Idents(Newmacula_rep) <- "celltypes"
DefaultAssay(Newmacula_rep) <- 'RNA'
VlnPlot(Newmacula_rep, features = c("ENSMFAG00000036911", "SLC17A6", "SNCG","SYT2","GFP"), pt.size = 0.01, ncol=3, combine = TRUE)
```

```{r, eval = FALSE}
qsave(Newmacula_rep, file = "Newmacula_rep.qs")
```
...
```{r}
Newmacula <- qread("Newmacula.qs")
```

```{r, eval = FALSE}
Newmacula <- SCTransform(Newmacula, method = "glmGamPoi", vst.flavor = "v2",
        vars.to.regress = c("percent.mt", "S.Score", "G2M.Score"), verbose = TRUE)

Newmacula <- RunPCA(Newmacula, npcs = 40)
# ElbowPlot(seuratObj, ndims = 40)
Newmacula <- RunUMAP(Newmacula, reduction = "pca", dims = 1:40)
Newmacula <- FindNeighbors(Newmacula, reduction = "pca", dims = 1:40)
Newmacula <- FindClusters(Newmacula, resolution = 0.1)
table(Newmacula$seurat_clusters)

Newmacula <- sctype(Newmacula)
```

```{r, fig.height=10, fig.width=12}
DimPlot(Newmacula, reduction = "umap", label = TRUE, repel = TRUE, group.by = 'celltypes') + theme(legend.position = "top")
DimPlot(Newmacula, reduction = "umap", group.by = "sample") + theme(legend.position = "top")
DimPlot(Newmacula, reduction = "umap", label = TRUE, split.by = "sample", group.by = 'celltypes', ncol = 4) + theme(legend.position = "top")
Idents(Newmacula) <- "celltypes"
DefaultAssay(Newmacula) <- 'RNA'
VlnPlot(Newmacula, features = c("ENSMFAG00000036911", "SLC17A6", "SNCG","SYT2","GFP"), pt.size = 0.01, ncol=3, combine = TRUE)
```

```{r, eval = FALSE}
qsave(Newmacula, file = "Newmacula.qs")
```


#### Periphery  
```{r}
periphery<- qread("periphery.qs")
```

```{r, eval = FALSE}
periphery <- SCTransform(periphery, method = "glmGamPoi", vst.flavor = "v2",
        vars.to.regress = c("percent.mt", "S.Score", "G2M.Score"), verbose = TRUE)

periphery <- RunPCA(periphery, npcs = 40)
# ElbowPlot(seuratObj, ndims = 40)
periphery <- RunUMAP(periphery, reduction = "pca", dims = 1:40)
periphery <- FindNeighbors(periphery, reduction = "pca", dims = 1:40)
periphery <- FindClusters(periphery, resolution = 0.2)
table(periphery$seurat_clusters)

periphery <- sctype(periphery)
```

```{r, fig.height=10, fig.width=12}
DimPlot(periphery, reduction = "umap", label = TRUE, repel = TRUE, group.by = 'celltypes') + theme(legend.position = "top")
DimPlot(periphery, reduction = "umap", group.by = "sample") + theme(legend.position = "top")
DimPlot(periphery, reduction = "umap", label = TRUE, split.by = "sample", group.by = 'celltypes', ncol = 4) + theme(legend.position = "top")
Idents(periphery) <- "celltypes"
DefaultAssay(periphery) <- 'RNA'
VlnPlot(periphery, features = c("ENSMFAG00000036911", "SLC17A6", "SNCG","SYT2","GFP"), pt.size = 0.01, ncol=3, combine = TRUE)
```

```{r, eval = FALSE}
qsave(periphery, file = "periphery.qs")
```
...
```{r, eval = FALSE}
Idents(Newmacula_rep) <- "celltypes"
Newmacula_rep_RGCs <- subset(Newmacula_rep, idents = "Retinal ganglion cells")

Idents(Newmacula) <- "celltypes"
Newmacula_RGCs <- subset(Newmacula, idents = "Retinal ganglion cells")

Idents(periphery) <- "celltypes"
periphery_RGCs <- subset(periphery, idents = "Retinal ganglion cells")

cat('Number of RGCs: \n')
dim(Newmacula_rep_RGCs)[2]
dim(Newmacula_RGCs)[2]
dim(periphery_RGCs)[2]
```

## Integrate RGCs from Newmacula_rep, Newmacula and periphery
```{r, eval = FALSE}
## Split object and SCTransform normalization  
RGC.list <- c(SplitObject(Newmacula_rep_RGCs, split.by = "sample"), SplitObject(Newmacula_RGCs, split.by = "sample"), SplitObject(periphery_RGCs, split.by = "sample"))
...
# add groups
group <- ifelse(grepl("GTSAMPLE1702", RGC.integrated$sample), "Newmacula_rep",
            ifelse(grepl('GTSAMPLE1488', RGC.integrated$sample), "periphery", "Newmacula"))
...
# try 0.3-0.5, give 26, 28, 30 clusters
RGC.integrated <- FindClusters(RGC.integrated, resolution = 0.05)
table(RGC.integrated$seurat_clusters)
```
...
genes <- c("NEFH", "SNCG", "NEFL", "SLC17A6", "THY1", "MEIS2", "GFP")
VlnPlot(RGC.integrated, features = genes)

# new markers
VlnPlot(RGC.integrated, features = c("ELAVL2", "RBFOX3", "IRX1","IRX2"))
```
...
cluster_to_celltype <- c('0' = 'Midget OFF',  '1' = 'Midget ON', '2' = 'Other', 
    '3' = 'Other', '4' = 'Parasol OFF', '5' = "Parasol ON", '6' = "Other", '7' = 'Other')
...
gfp_pos_GRC <- subset(RGC.integrated, subset = GFP > 0.5)
gfp_neg_GRC <- subset(RGC.integrated, subset = GFP < 0.5)
...
```{r}
DefaultAssay(RGC.integrated) <- "RNA"
gfp_expression <- FetchData(RGC.integrated, vars = "GFP")
RGC.integrated$GFP_label <-  ifelse(gfp_expression > 0.5, "GFP+", "GFP-")
