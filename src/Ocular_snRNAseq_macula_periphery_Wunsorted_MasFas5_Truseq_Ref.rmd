---
title: "Ocular_snRNAseq_macula_periphery_Wunsorted_MasFas5_Truseq_Ref"
output:
  html_document:
    toc: true
    toc_float: true
...
# Abstract
The first 6 samples are basically a replicate from last week. Samples 7 and 8 are "unsorted" macula lanes. 
The macula tissue seemed to have less debris, so Kamil and I thought it might be useful to compare sorted versus 
unsorted reactions for this tissue (the primary readout being barcode rank plots). Other than that, the 
computational goals should be very similar to the previous experiment. The sequencing folder is 
240124_VH00163_80_AACCNCGHV
The peripheral region is different in this experiment. Rather than nasal, it's temporal. See the table below for sample info from this experiment:   

GTSAMPLE1044-sorted-1	FR2749M Macular Retina Punch OS - Lane 1  
GTSAMPLE1044-sorted-2	FR2749M Macular Retina Punch OS - Lane 2  
GTSAMPLE1044-sorted-3	FR2749M Macular Retina Punch OS - Lane 3  
GTSAMPLE1045-1	FR2749M Inferotemporal Retina OS - Lane 1  
GTSAMPLE1045-2	FR2749M Inferotemporal Retina OS - Lane 2  
GTSAMPLE1045-3	FR2749M Inferotemporal Retina OS - Lane 3  
GTSAMPLE1044-unsorted-1	FR2749M Macular Retina Punch OS - Unsorted Lane 1  
GTSAMPLE1044-unsorted-2	FR2749M Macular Retina Punch OS - Unsorted Lane 2  

sample 1 (sorted macula replicate 1) didn't really amplify. Sample 2 had clog issue, should not included either.ß
...
setwd('~/Ocular_snRNAseq_macula_periphery_Wunsorted_MasFas5_Truseq_Ref/')
set.seed(123)
```
...
rds_path <- rds_path[3:8]
rds_names <-  gsub("_matrix.rds$", "", basename(rds_path))
rds_list <- lapply(rds_path, readRDS)
names(rds_list) <- rds_names

# combine samples
seuratObj <- merge(rds_list[[1]], y = rds_list[2:6], add.cell.ids = names(rds_list), project = "Ocular_macula_periphery")
# add sample names
...
sample = c(
"GTSAMPLE1044-sorted-3",
"GTSAMPLE1045-1",
"GTSAMPLE1045-2",
"GTSAMPLE1045-3",
"GTSAMPLE1044-unsorted-1",
"GTSAMPLE1044-unsorted-2"
)

sample_names = c(
"FR2749M_Macular_Retina_Punch_OS-3",
    "FR2749M_Inferotemporal_Retina_OS-1",
    "FR2749M_Inferotemporal_Retina_OS-2",
    "FR2749M_Inferotemporal_Retina_OS-3",
    "FR2749M_Macular_Retina_Punch_OS-Unsorted-1",
    "FR2749M_Macular_Retina_Punch_OS-Unsorted-2")

name_mapping <- setNames(sample_names, sample)
seuratObj@meta.data$sample <- name_mapping[seuratObj@meta.data$sample]
#add groups
tissue_info <- ifelse(grepl("Macular", seuratObj$sample), "Macular", "Inferotemporal")
cell_source <- ifelse(grepl("Unsorted", seuratObj$sample), "Unsorted", "Sorted")
seuratObj <- AddMetaData(seuratObj, metadata = tissue_info, col.name = "tissue")
seuratObj <- AddMetaData(seuratObj, metadata = cell_source, col.name = "cell_source")
```
...
```{r, fig.height=7, fig.width=14}
p1 <- DimPlot(seuratObj, reduction = "umap", group.by = "tissue") + theme(legend.position = "top")
p2 <- DimPlot(seuratObj, reduction = "umap", group.by = "cell_source") + theme(legend.position = "top")
p1 + p2
```
...
VlnPlot(sn, features = c("RBPMS", "SLC17A6", "SNCG","SYT2"), pt.size = 0.01, ncol=2, combine = TRUE)

VlnPlot(sn, features = c("OPN1LW", "OPN1SW"), pt.size = 0.01, ncol=2, combine = TRUE)
```
...
percentage_data <- sn@meta.data %>%
    ...
    ungroup() %>%
    mutate(sample = str_replace(sample, "FR2749M_", ""))
