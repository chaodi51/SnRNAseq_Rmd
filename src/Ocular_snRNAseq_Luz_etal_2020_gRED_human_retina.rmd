---
title: "Ocular_snRNAseq_Luz_etal_2020_gRED_human_retina"
output:
  html_document:
    toc: true
    toc_float: true
...
setwd('~/Ocular_snRNAseq_publicdata_GRK1/human/Luz_etal_2020_gRED_human_retina')
set.seed(123)
```
...
```{r, eval = FALSE}
file_paths <- list.files('.', pattern = "\\.txt.gz", full.names = TRUE)
file_names <-  sub(".*_([^_]+)\\.txt.*", "\\1", file_paths)
...
# load metadata table
metadata <- read.table('GSE135133_clusterAssignments.txt', sep="\t", header = TRUE)
metadata <- metadata %>% select(cellID, Tissue, Sex, Cell_type_full, Clusters_full)
retina <- AddMetaData(retina, metadata)
...
colnames(retina@meta.data)[colnames(retina@meta.data) == "Cell_type_full"] <- "cell_type"
```
...
DotPlot(subset(retina, idents = as.vector(na.omit(unique(retina$cell_type)))), features = label_genes, cols = c("white", 'blue'), dot.scale = 8) + RotatedAxis() + coord_flip()
```
