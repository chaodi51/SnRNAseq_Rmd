---
title: "Ocular snRNAseq pilot_dataset1+dataset3"
output:
  html_document:
    toc: true
    toc_float: true
...
setwd('~/Ocular_snRNAseq_pilot_3rd/integrate_set1_and_set3/')
set.seed(123)
```
...
# combine samples
seuratObj <- merge(rds_list[[1]], y = rds_list[2:8], add.cell.ids = names(rds_list), project = "ocular_pilot")
# add sample names
cells <- rownames(seuratObj@meta.data)

tmp <- sub("(NHP_S_[1-4]).*", "\\1", cells)
seuratObj$sample <- sub("(NHP-1_[1-4]).*", "\\1", tmp)

#add groups
dataset_info <- ifelse(grepl("NHP_S", seuratObj$sample), "rep1", ifelse(grepl("NHP-1", seuratObj$sample), "rep2", "unknown"))
