---
title: "Ocular snRNAseq pilot (unsorted samples)"
output:
  html_document:
    toc: true
    toc_float: true
...
rds_paths <- list.files('./nf-core/output/cellranger/cellranger/mtx_conversions/',
    pattern = "NHP_NS_[1-4]_matrix\\.rds", full.names = TRUE)
rds_names <-  gsub("_matrix.rds$", "", basename(rds_paths))
rds_list <- lapply(rds_paths, readRDS)
names(rds_list) <- rds_names
# combine samples
seuratObj <- merge(rds_list[[1]], y = rds_list[2:4], add.cell.ids = rds_names, project = "ocular_pilot_NS")
# add sample names
cells <- rownames(seuratObj@meta.data)

seuratObj$sample <- sub("(NHP_NS_[1-4]).*", "\\1", cells)
```
...
```{r}
seuratObj <- readRDS("ocular_obj_NS.rds")
```
...
```{r, eval = FALSE}
saveRDS(seuratObj, file = "ocular_obj_NS.rds")
```
