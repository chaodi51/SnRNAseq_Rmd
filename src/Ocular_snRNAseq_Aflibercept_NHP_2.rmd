...
# Abstract
Eight samples in this run to be technical replicates (from the back of the eye), 2nd round with dissection.
...
setwd('/home/ubuntu/Ocular_snRNAseq_Aflibercept_NHP_2')
set.seed(123)
```
...
rds_path <- list.files('./nf-core/output/cellranger/cellranger/mtx_conversions/',
    pattern = "GTSAMPLE0869-[0-9]_matrix\\.rds", full.names = TRUE)
...
seuratObj$tissue <- "posterior_dissect"
```
...
```{r, eval = FALSE}
seuratObj <- subset(seuratObj, subset = percent.mt < 3 &
    nFeature_RNA > 100 &
    nCount_RNA > 200)
```
...
perc_exp <- Percent_Expressing(seurat_object = seuratObj, features = c("Aflibercept", "ENSMFAG00000036911", "SLC17A6", "SNCG", "SYT2"))
...
# dev.off()
# appears that clusters 15 and 32 can be identified as RGCs
```